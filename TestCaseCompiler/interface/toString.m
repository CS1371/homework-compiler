%% toString: Represent any MATLAB datatype as a string
%
% toString will generate a character vector representation of any MATLAB
% datatype.
%
% S = toString(D) will take data D of any type and produce a string
% representation S.
%
% S = toString(D, C) generates a string of length at most C.
%
%%% Remarks
%
% This function overloads rptgen.toString() by representing logical scalars
% as 'true' and 'false', respectively, and by removing newline characters
% from multiline representations. Also puts single quotes around strings.
%
function [str] = toString(in, numChars)
str = '';
if islogical(in)
    if in
        str = 'true';
...
elseif ischar(in) && size(in, 1) == 1
   ...
   str = toString_rptgen(in, numChars);
   % remove all newlines from the result of rptgen.toString
   str = str(str ~= sprintf('\n'));
    
end


function s = toString_rptgen(v, varargin)
