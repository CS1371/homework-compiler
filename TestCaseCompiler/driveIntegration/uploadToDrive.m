...
function uploadToDrive(path, folder, token, key, progress)
    ...
    % repeat.
    workers = uploadFolder(path, newId, token);
...
function workers = uploadFolder(path, newId, token)
    folders = dir(path);
    folders(~[folders.isdir]) = [];
    folders(strncmp({folders.name}, '.', 1)) = [];
    workers = cell(1, numel(folders) + 1);
    % for each folder, rinse and repeat
    for f = 1:numel(folders)
        % create the folder
        subId = createFolder(newId, folders(f).name, token);
        workers{f} = uploadFolder([folders(f).folder filesep folders(f).name], subId, token);
    end
    ...
    for a = numel(attachments):-1:1
        workers{end}(a) = parfeval(@uploadFile, 0, attachments{a}, newId, token);
    end
    workers = [workers{:}];
end
